...
<title>列表</title>
<link rel="stylesheet" type="text/css" href="${appContext.appPath}/function/js/easyui/themes/default/easyui.css" />
<link rel="stylesheet" type="text/css" href="${appContext.appPath}/function/js/easyui/themes/icon.css" />
<link rel="stylesheet" type="text/css" href="${appContext.skinPath}/css/gzt.css" type="text/css"></link>
<link rel="stylesheet" type="text/css" href="${appContext.skinPath}/css/index.css" type="text/css"></link>
<script type="text/javascript">var APP_PATH = "${appContext.appPath}";</script>
<script type="text/javascript" src="${appContext.skinPath}/js/jquery-1.9.1.min.js"></script>
...
<script type="text/javascript" src="${appContext.appPath}/function/js/easyui/jquery.easyui.min.js"></script>
<script type="text/javascript" src="${appContext.appPath}/function/js/easyui/easyui-lang-zh_CN.js"></script>
<script type="text/javascript" src="${appContext.appPath}/plugins/datePicker/WdatePicker.js"></script>
<script type="text/javascript">
        $(function () {
            ...
            $(".cx1").click(function(){
            	$.post("findByIdModule.action",{id : "${module.id}" },function(data){
            		if(data != ''){
    	...
    	function on_delete(subjectId){
    		if(confirm("确定要删除吗?")){
    			$.post("deleteModule.action", { id: subjectId},
    				function(data){
    					var dateObj=$.parseJSON(data);
    					if(0==dateObj.result){
    						document.location.href="findAllModule.action?module.id=${module.id}";
    					}else{
    						alert("删除失败");
    					}
    			});
    	...
    	function isSubjectExist(){
    		var url ="isExistModule.action";
    		var subjectTile = $("input[name='module.name']").val();
    		$.post(url,{name:subjectTile},function(data){
    				if(data.result == 0){
    						doSave();
    				}else{
    					alert("存在相同名称！");
    	...
    	function doSave(){
    		...
    		$("#saveForm").ajaxSubmit({
    			...
    			data: {executeUsersId:users.toString()},
    			success:function(data){
    				var dateObj=$.parseJSON(data);
    				alert((0==dateObj.result)?"成功!":"失败!");
    				document.location.href="findAllModule.action?module.parentId=tjfx";
    			}
    		});
    	}
...
<body>
<form action="findAllModule.action" id="form" method="post">
     ...
     <h1 class="title"><span class="title">当前位置：</span><span class="text">首页　-　<a href="${appContext.appPath}/scMaster2/teacherInput_index.action">校长工作台</a>　-　</span><span class="back">临时事项</span></h1>
        ...
        <table width="92%" border="0">
            ...
            <tr>
                ...
                <td align="center">${sub.createUserId}</td>
                <td align="center">
